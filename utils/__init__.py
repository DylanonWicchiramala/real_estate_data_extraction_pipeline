...
@bundle_input
def filter_dict(data: dict, keys_to_keep: list[str]=None, key_to_exclude: list[str]=None) -> dict:
    """ filter dictionary by keys."""
    
    if keys_to_keep:
        filtered_item = {key: value for key, value in data.items() if key in keys_to_keep}
    elif key_to_exclude:
        filtered_item = {key: value for key, value in data.items() if key not in key_to_exclude}
